...
struct queue_entry {
  ...
  u8  *frontier_node_bitmap;      /*用于记录每个种子覆盖的前沿节点*/;      /*用于记录每个种子覆盖的前沿节点*/
  u32 *covered_frontier_node_list;  // 仅存储该种子实际覆盖的 frontier nodes (edge_id)
...
#define N_FUZZ_SIZE (1 << 21)
  ...
  u32 covered_favored_seed_list_counter;
  u32 unsearch_global_frontier_bitmap_count;


  u8 *global_frontier_bitmap;  // 定义全局frontier nodes位图
  u8 *global_frontier_bitmap_temp;
  u8 *global_frontier_bitmap_searched;
  ...
  u64 total_crashes,                    /* Total number of crashes          */
      ...
      baseline_global_cull_queue_time,
      setcover_global_cull_queue_time,
      random_builtin_time,
      setcover_cost_time,
      random_cost_time;
...
double calculate_priority_score(afl_state_t *afl, struct queue_entry *q);
