...
int main(int argc, char **argv_orig, char **envp) {
  ...
  if (afl->use_set_cover_scheduling) {
    ...
    u32 alloc_size = (afl->fsrv.real_map_size >> 3) + 1;
    
    afl->global_frontier_bitmap = ck_alloc(alloc_size);
    afl->global_frontier_bitmap_temp = ck_alloc(alloc_size);
    afl->global_frontier_bitmap_searched = ck_alloc(alloc_size);
...
// -----------------------------------------------main-------------------------------------------
  while (likely(!afl->stop_soon)) {
    ...
    if (unlikely((!afl->old_seed_selection &&
      ...
      if (unlikely(afl->queued_items == prev_queued // todo 
                   /* FIXME TODO BUG: && (get_cur_time() - afl->start_time) >=
                      3600 */
                   )) {
        ACTF("we had a full queue cycle with no new finds");
        if (afl->use_splicing) {
      ...
      if (afl->cycle_schedules) {
        ACTF("afl->cycle_schedules is true");
    ...
    do {
      ...
      // fprintf(afl->fsrv.set_cover_analysis, "++++++++++++++++++++++++++\n" );
      ...
      if (likely(afl->pending_favored && afl->smallest_favored >= 0)) {
        ...
        */
        ...
        do {
          ...
          fprintf(afl->fsrv.set_cover_analysis, "select_next_queue_entry : %u\n",afl->current_entry);


        } while (unlikely(afl->current_entry >= afl->queued_items) || !afl->queue_buf[afl->current_entry]->set_covered);

        afl->queue_cur = afl->queue_buf[afl->current_entry];
        fprintf(afl->fsrv.set_cover_analysis, "final_entry : %u, setcovered : %u\n",afl->current_entry,afl->queue_buf[afl->current_entry]->set_covered);
      ...
      if (!afl->use_set_cover_scheduling) {
        ...
        fprintf(afl->fsrv.set_cover_analysis, "%u %u %u\n",afl->covered_seed_list_counter, afl->queued_items,afl->set_favored_id);
...
stop_fuzzing:
  ...
  if (afl->not_on_tty) {
    ...
    fprintf(afl->fsrv.set_cover_analysis, "cull_queue_total_time: %llu\n",afl->setcover_global_cull_queue_time);
