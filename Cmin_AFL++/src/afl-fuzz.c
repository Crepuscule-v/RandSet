...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (likely(!afl->stop_soon)) {
    time_t _cmin_t0 = time(NULL);
    sched_cmin_maybe_run(afl);
    time_t _cmin_t1 = time(NULL);

    if (_cmin_t1 >= _cmin_t0) {
      afl->total_cmin_time += (u64)(_cmin_t1 - _cmin_t0); 
    }
    ...
    if (skipped_fuzz == 0)
    {
        fprintf(afl->fsrv.plot_file, "selected seed : [%u]\n", afl->current_entry); /* ignore errors */
