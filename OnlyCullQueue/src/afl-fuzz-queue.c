...
void cull_queue(afl_state_t *afl) {
  ...
  for (i = 0; i < afl->queued_items; i++) {
    ...
    afl->queue_buf[i]->perf_score = calculate_score(afl, afl->queue_buf[i]);
  ...
  if (afl->smallest_favored < 0) {

    afl->smallest_favored = afl->last_smallest_favored;

  } else {
    
    afl->last_smallest_favored = afl->smallest_favored;
