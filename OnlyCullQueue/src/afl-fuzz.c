...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      fprintf(afl->fsrv.set_cover_analysis, "%u %u %u %u\n",afl->queued_items, afl->queued_favored, afl->current_entry, skipped_fuzz );
...
stop_fuzzing:
  ...
  if (afl->not_on_tty) {
    ...
    fprintf(afl->fsrv.set_cover_analysis, "queue_items: %u queued_favored: %u \n",afl->queued_items, afl->queued_favored);
